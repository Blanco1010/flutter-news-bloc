...
class NewsListPaginationScreen extends StatefulWidget {
  ...
  final String selectedCountries;

  const NewsListPaginationScreen({Key? key, required this.selectedCountries})
      : super(key: key);
      ...
      body: BlocBuilder<NewsBloc, NewsState>(
        builder: (context, state) {
          if (state is NewsPageLoadingMore) {
            ...
            if (_newsList.length >= (state.data?.totalResults ?? 0)) {
      ...
      controller: _scrollController
        ..addListener(() {
          if (_scrollController.offset ==
              ...
              _newsBloc.haveMoreData) {
            _newsBloc.haveMoreData = false;
            _newsBloc.add(
              NewsFetchDataPaginationEvent(selectedCountries, newsType),
            );
          }
        }),
      children: [
